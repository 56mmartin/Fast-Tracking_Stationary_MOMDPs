...
SparseVector *backupInitialBelief = new SparseVector;   // because initialBeliefY will be temporarily modified.
copy(*backupInitialBelief, *(problem->initialBeliefY));

FOR(fixedModel, 1 + problem->initialBeliefY->data.size())
//FOR(fixedModel, 1)
{
	if (fixedModel <  problem->initialBeliefY->data.size() && 0.00000001 > backupInitialBelief->data[fixedModel].value)
		continue;

//	if (fixedModel <  problem->initialBeliefY->data.size() && problem->adaptiveManagement == false)
	if (fixedModel <  problem->initialBeliefY->data.size())
		continue;

	if (fixedModel <  problem->initialBeliefY->data.size())
	{
		cout << "Model #" << backupInitialBelief->data[fixedModel].index << endl;
		FOR(model, problem->initialBeliefY->data.size())
		{
			if (fixedModel == model)
				problem->initialBeliefY->data[model].value = 1;  // set initial belief to one model only
			else
				problem->initialBeliefY->data[model].value = 0;
		}
	...
	else
	{
		...
		FOR(model, problem->initialBeliefY->data.size())
		{
			problem->initialBeliefY->data[model].value = backupInitialBelief->data[model].value;
			problem->initialBeliefY->data[model].index = backupInitialBelief->data[model].index;
