		...
		FOR (a, pomdp->actions->size())
		{
			...
			do
			{
				...
				FOR (sval, numStatesObs ) {	// for each sval, get nextAl[sval]
                   ...
                   // cout << 44 << pomdp->XTrans->getMatrix(a, sval)->size1() << endl;
                    //cout << 44 << (*pomdp->XTrans->getMatrix(a, sval))[0] << endl;
                   // int zzz; cin >> zzz;
					...
					FOREACH (int, nextSvalIt, possibleNextSvals)
                    {
                        ...
                        // transition on y given the transition on x:
                        mult(tmpNextAl, *pomdp->YTrans->getMatrix(a, sval, nextSval), als[nextSval]);   // tmpNextAl = mat * nextAlpha;
                        // Does PAM consist of skipping the above line?


                        // transition on x given the transition on y:
                        emult_column( tmpNextAl1, *pomdp->XTrans->getMatrix(a, sval), nextSval, tmpNextAl );   //tmpNextAl1 = tmpNextAl.*mat(:,nextSval);
                        tmpAl += tmpNextAl1;
                    }
					...
					nextAls[sval] = tmpAl;   // update the unique alpha vector for one x.
				}
			...
			FOR (sval, numStatesObs )
			{
                ...
                // once the alphavector is calculated, we create the alphaplane, which contains a little more info.
				SharedPointer<AlphaPlane> plane (new AlphaPlane());
				copy(*plane->alpha, als[sval]);
				plane->action = a;
				plane->sval = sval;
