...
void BeliefForest::globalRootPrepare(void)
{
	...
	FOR(r, rootpv.size())
	{
		...
		if (rprob > OBS_IS_ZERO_EPS)
		{
			...
			thisRootb_s->sval = r;
			rE->sampleRoot = sampleEngine->getNode(thisRootb_s);  // kind of copies thisRootb_s into rE->sampleRoot.

			double lb = (*beliefCacheSet)[rE->sampleRoot->cacheIndex.sval]->getRow(rE->sampleRoot->cacheIndex.row)->LB;
//			cout << (*beliefCacheSet)[rE->sampleRoot->cacheIndex.sval]->getRow(rE->sampleRoot->cacheIndex.row)->LB << endl;
//			cout <<44<< endl;
